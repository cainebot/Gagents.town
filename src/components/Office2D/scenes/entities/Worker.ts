/**
 * Worker (NPC agent) entity with movement, idle behavior, tasks, and emotes.
 * Extracted from Agent Town: components/game/entities/Worker.ts
 */
...
export class Worker implements WorkerCtx {
  ...
  // -- Delegated: Movement --
  navigateTo(x: number, y: number, facePoi?: { x: number; y: number }) { movNavigateTo(this, x, y, facePoi); }
  navigateHome() { movNavigateHome(this); }
  isAtHomePose() { return movIsAtHomePose(this); }

  // -- Delegated: Task management --
  assignTask(runId: string, taskMessage: string, onReady?: () => void) { taskAssignTask(this, runId, taskMessage, onReady); }
  completeTask() { taskCompleteTask(this); }
  failTask() { taskFailTask(this); }
  abortTask(runId: string) { return taskAbortTask(this, runId); }
  enqueueTask(runId: string, message: string, onReady?: () => void) { taskEnqueueTask(this, runId, message, onReady); }

  // -- Delegated: Idle behavior --
  stopIdleActivity() { idleStopIdleActivity(this); }
  ...
  // -- Public helpers --
  setPOIs(pois: POI[]) { this.pois = pois; }
  setPathfinder(pf: Pathfinder) { this.pathfinder = pf; }
  canInteract() { return !this.interactionLocked; }
  isAwayFromDesk() { return this.moveTarget !== null || this.isWandering || !this.isAtHomePose(); }
