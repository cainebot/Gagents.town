...
export async function POST(request: NextRequest) {
  ...
  const { title, type, target_agent_id, source_agent_id, priority, payload, max_retries } = body
  ...
  if (!resolvedTargetAgentId) {
    ...
    if (agents && agents.length > 0) {
      // 2. Count in-flight tasks (pending, claimed, in_progress) per agent
      ...
      // 3. Prefer idle agents, then pick the one with fewest in-flight tasks
      const idleAgents = agents.filter((a: { status: string }) => a.status === 'idle')
      const candidates = idleAgents.length > 0 ? idleAgents : agents
